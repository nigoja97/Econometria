...
plot(TRM$f[-1],diff(log(TRM$Precio)),ylab='Cambio en Log(Precio)',type='l')
...
TRM$Trmma = ma(TRM$Precio, order=7)
TRM$Trmma30 = ma(TRM$Precio, order=30)
...
TRmma = ts(na.omit(TRM$Precio), frequency=365)
...
plot(decomp)
plot(deseasonal_trm)
plot(TRM$Precio,deseasonal_trm)
...
adf.test(TRM$Precio, alternative = "stationary")
...
adf.test(diff(TRM$Precio), alternative = "stationary")
...
par(mfrow=c(2,1))
acf(diff(TRM$Precio))
pacf(diff(TRM$Precio))

# dif log, variaciones relativas
par(mfrow=c(2,1))
par(mar=c(2,4,1,1))
acf(diff(log(TRM$Precio)))
pacf(diff(log(TRM$Precio)))
...
fit2 = arima(TRM$Precio, order=c(2,1,1))
...
primero = arima(TRM$Precio, order=c(2,1,3))
primero
...
segundo = arima(TRM$Precio, order=c(2,1,2))
segundo
...
tercero = arima(TRM$Precio, order=c(2,1,1))
tercero
...
auto.arima(TRM$Precio, seasonal=FALSE)

fit <- auto.arima(TRM$Precio)
fit
par(mar=c(2,2,1,1))
...
TRMT=ts(TRM$Precio,start=c(2005,05,01),freq=365)
